...
from yeschef.cli import codex_config_with_yeschef
...
def test_codex_config_appends_the_server_block() -> None:
    updated = codex_config_with_yeschef('model = "gpt-5"\n', "http://mini:8787")
    assert "[mcp_servers.yeschef]" in updated
    assert 'command = "yeschef"' in updated
...
def test_codex_config_is_idempotent() -> None:
    once = codex_config_with_yeschef("", "http://h:8787")
    assert codex_config_with_yeschef(once, "http://h:8787") is None
...
def test_codex_config_normalizes_a_missing_trailing_newline() -> None:
    ...
    updated = codex_config_with_yeschef("model = 'x'", "http://h:8787")
    assert "\n[mcp_servers.yeschef]" in updated
